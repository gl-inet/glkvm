...
export function Hid(__getGeometry, __recorder) {
	...
	var __init__ = function() {
		...
		tools.el.setOnClick($("udc-reinit-button"), __clickUdcReinitButton);
	...
	var __clickUdcReinitButton = function() {
		wm.confirm("Are you sure you want to reinitialize USB (UDC)?<br>This will temporarily disconnect all USB devices.").then(function(ok) {
			if (ok) {
				tools.httpPost("api/system/reinit_udc", null, function(http) {
					if (http.status !== 200) {
						wm.error("UDC reinit error", http.responseText);
					}
				});
			}
		});
