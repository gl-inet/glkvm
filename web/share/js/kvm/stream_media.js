...
export function MediaStreamer(__setActive, __setInactive, __setInfo, __organizeHook, __orient) {
	...
	var __ping = function() {
		try {
			...
			if (__decoder && __decoder.state === "configured") {
				...
				let info = `${current_fps} fps (avg: ${avg_fps.toFixed(1)}, total: ${__total_frames}, overall: ${total_avg_fps.toFixed(1)})`;
				__fps_accum = 0;
	...
	var __setupCodec = function(formats) {
		...
		if (!window.VideoDecoder) {
			let msg = "This browser can't handle direct video stream";
			if (!tools.is_https) {
				msg = "Direct streaming requires HTTPS";
			}
		...
		let desired_fmt = "h264";
		if (__state && __state.video_format === 1) {
			desired_fmt = "h265";
		}

		let chosen_fmt = null;
		let chosen_meta = null;
		for (let fmt of [desired_fmt, "h264", "h265", "jpeg"]) {
			if (formats[fmt] !== undefined) {
				chosen_fmt = fmt;
				chosen_meta = formats[fmt];
				break;
			}
		}

		if (!chosen_fmt) {
			let msg = "No suitable stream available on PiKVM";
			__setInfo(false, false, msg);
			__logInfo(msg);
			return;
		}
		...
		let codec = null;
		if (chosen_meta) {
			// Backend may send ready-to-use codec string
			if (chosen_meta.codec) {
				codec = chosen_meta.codec;
			} else if (chosen_meta.profile_level_id) {
				if (chosen_fmt === "h264") {
					codec = `avc1.${chosen_meta.profile_level_id}`;
				} else if (chosen_fmt === "h265") {
					codec = `hvc1.${chosen_meta.profile_level_id}`;
				}
			}
		}

		if (!codec) {
			let msg = `Unsupported codec info for ${chosen_fmt}`;
			__setInfo(false, false, msg);
			__logInfo(msg);
			return;
		}
		...
		__ws.send(JSON.stringify({
			"event_type": "start",
			"event": {"type": "video", "format": chosen_fmt},
		}));
	};
